...
NiiReturnStatus cMosNii::configureTransfer(PublicId tc_id,
                                  const NetworkDescriptor& network_id,
                                  TransferDirection send_receive,
                                  TransferType message_streaming,
                                  TC_ConfigurationData configuration_data,
                              ...
                              Time time_out)
{
  ...
  else
  {
    ...
    do
    {
		bytes = sendto(tc->fd, transmit_data, data_length, 0, (struct sockaddr *) &addr, sizeof(addr));
...
NiiReturnStatus cMosNii::receiveTransfer( PublicId tc_id, 
                                          char* receive_data,
                                          Length data_length_available,
                                          Length& data_length,
                                          Time time_out)
{
  Index tc_idx;
...
NiiReturnStatus cMosNii::receiveNetwork(const NetworkDescriptor& network_id, char* receive_data, Length data_length_available, Length& data_length, PublicId& tc_id, Time time_out)
{
    fd_set    rfds;
    struct   timeval tv;
    int       retval;
...
char* cMosNii::spell(NiiReturnStatus status)
{
  switch(status)
  {
    case ASAAC_MOS_NII_CALL_FAILED:         return "ASAAC_MOS_NII_CALL_FAILED";
    case ASAAC_MOS_NII_CALL_COMPLETE:       return "ASAAC_MOS_NII_CALL_COMPLETE";
    case ASAAC_MOS_NII_CALL_OK:             return "ASAAC_MOS_NII_CALL_OK";
    case ASAAC_MOS_NII_INVALID_INTERFACE:   return "ASAAC_MOS_NII_INVALID_INTERFACE";
    case ASAAC_MOS_NII_INVALID_CONFIG:      return "ASAAC_MOS_NII_INVALID_CONFIG";
    case ASAAC_MOS_NII_INVALID_NETWORK:     return "ASAAC_MOS_NII_INVALID_NETWORK";
    case ASAAC_MOS_NII_INVALID_TC:          return "ASAAC_MOS_NII_INVALID_TC";
    case ASAAC_MOS_NII_INVALID_MESSAGE_SIZE:return "ASAAC_MOS_NII_INVALID_MESSAGE_SIZE";
    case ASAAC_MOS_NII_OPEN_TCS:            return "ASAAC_MOS_NII_OPEN_TCS";
    case ASAAC_MOS_NII_ALREADY_CONFIGURED:  return "ASAAC_MOS_NII_ALREADY_CONFIGURED";
    case ASAAC_MOS_NII_TC_NOT_CONFIGURED:   return "ASAAC_MOS_NII_TC_NOT_CONFIGURED";
    case ASAAC_MOS_NII_STORAGE_FAULT:       return "ASAAC_MOS_NII_STORAGE_FAULT";
    case ASAAC_MOS_NII_BUFFER_EMPTY:        return "ASAAC_MOS_NII_BUFFER_EMPTY";
    case ASAAC_MOS_NII_BUFFER_NOT_READY:    return "ASAAC_MOS_NII_BUFFER_NOT_READY";
    case ASAAC_MOS_NII_STATUS_ERROR:        return "ASAAC_MOS_NII_STATUS_ERROR";
    case ASAAC_MOS_NII_STATUS_INIT:         return "ASAAC_MOS_NII_STATUS_INIT";
    case ASAAC_MOS_NII_STATUS_OK:           return "ASAAC_MOS_NII_STATUS_OK";
    
    default: return "MOS NII ERROR: undefined return status";
  }
}
...
void* cMosNii::streamTcThread(void* pTcData)
{
  ...
  if(tc->direction == ASAAC_TRANSFER_DIRECTION_SEND)
  {
    ...
    while(connect(tc->fd,(const sockaddr*) &server,sizeof(struct sockaddr_in)) < 0)
    {
#ifdef _DEBUG_
...
#endif
        written_bytes = write(tc->fd,tc->config_data.stream_buffer+os,*tc->config_data.data_length);
        if(written_bytes < 0)
        {
#ifdef _DEBUG_
...
#endif
      int client_socket = accept(tc->fd,(sockaddr*) &client,(socklen_t*) &clnt_len);
      int read_bytes = -1;
...
ASAAC_NiiReturnStatus ASAAC_MOS_configureInterface(const ASAAC_PublicId interface_id, 
                                                   const ASAAC_NetworkDescriptor* network_id, 
                                                   const ASAAC_InterfaceConfigurationData* configuration_data)
{
    return cMosNii::getInstance()->configureInterface(interface_id, *network_id, *configuration_data);
};
                                                  ...
                                                  const ASAAC_PublicId callback_id)
{
    return cMosNii::getInstance()->configureTransfer(
        tc_id,              //    PublicId - const ASAAC_PublicId
...
ASAAC_NiiReturnStatus ASAAC_MOS_receiveTransfer(const ASAAC_PublicId tc_id, ASAAC_CharAddress* receive_data, const ASAAC_Length data_length_available, ASAAC_Length* data_length, const ASAAC_Time* time_out)
{
     ...
     *receive_data = receiveBuffer[currentBuffer];
     ...
     return cMosNii::getInstance()->receiveTransfer( tc_id,
                                                      receiveBuffer[currentBuffer],
...
ASAAC_NiiReturnStatus ASAAC_MOS_receiveNetwork(const ASAAC_NetworkDescriptor* network, ASAAC_CharAddress* receive_data, const ASAAC_Length data_length_available, ASAAC_Length* data_length, ASAAC_PublicId* tc_id, const ASAAC_Time* time_out)
{
    ...
    *receive_data = receiveBuffer[currentBuffer];
     ...
     return cMosNii::getInstance()->receiveNetwork(*network, receiveBuffer[currentBuffer], data_length_available, *data_length, *tc_id, *time_out);
