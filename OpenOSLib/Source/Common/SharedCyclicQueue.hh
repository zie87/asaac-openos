...
public:
	 ...
	 *                            and whether the internal data structures shall be initialized, rather
	...
	//! push data into the next free cell of the queue
	...
	unsigned int getFreeCells() const;
	...
	static size_t predictSize( unsigned int Size );


	struct GlobalCyclicQueueData {
		unsigned int Size; 			// Total Size of Buffer (non-resizeable!)
		unsigned int Free;			// Number of free Cells in Buffer
		int NextWriteCell;			// Cell number into which the next Write operation will commence
		int NextReadCell;			// Cell number which will be read from in next pop() call
...
template <class T> void SharedCyclicQueue<T>::push( const T Value, const ASAAC_Time& Timeout )
{
	...
	// If a formerly empty cell was written into,
...
template <class T> ASAAC_TimedReturnStatus SharedCyclicQueue<T>::waitForFreeCells( const ASAAC_Time& Timeout )
{
	...
	// push into the queue still after this call has been done,
...
template <class T> unsigned int SharedCyclicQueue<T>::getFreeCells() const
{
	if ( ! m_IsInitialized ) throw UninitializedObjectException( LOCATION );
...
template <class T> void SharedCyclicQueue<T>::forceRelease()
{
	...
	int c = m_WriteSemaphore.getWaitingThreads();
	for (int i=0; i < c; i++ )
		m_WriteSemaphore.post();
...
template <class T> size_t SharedCyclicQueue<T>::predictSize( unsigned int Size )
{
