...
public:
	...
	Shared( Allocator* ThisAllocator, unsigned int Count = 1 );

	//! initialization method, called by initializing constructor
	virtual void initialize( Allocator* Allocator, unsigned int Count = 1 );
	...
	static size_t predictSize(unsigned int Count = 1);
...
template <class T> Shared<T>::Shared( Allocator* ThisAllocator, unsigned int Count ) 
...
template <class T> void Shared<T>::initialize( Allocator* ThisAllocator, unsigned int Count ) 
{
	m_Count = Count;
...
template <class T> inline T& Shared<T>::operator[]( int Index ) const 
{
	if (m_IsInitialized == false) 
		throw UninitializedObjectException(LOCATION);
...
template <class T> size_t Shared<T>::predictSize(unsigned int Count)
{
