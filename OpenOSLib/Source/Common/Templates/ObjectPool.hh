...
template <class T, unsigned long N> class ObjectPool {
	...
	public:
		...
		/*!< \param[in] Timeout Polong of time to wait until if no instances are instantly
...
template <class T, unsigned long N> ObjectPool<T,N>::ObjectPool()
{
	try
	{
...
template <class T, unsigned long N> void ObjectPool<T,N>::releaseObject( T* Object )
{
	if ( Object != 0 )
	{
