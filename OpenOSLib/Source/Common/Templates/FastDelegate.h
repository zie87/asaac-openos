...
//                  * New syntax: FastDelegate< long (char *, double) >. 
...
inline OutputClass horrible_cast(const InputClass input){
    ...
    typedef long ERROR_CantUseHorrible_cast[sizeof(InputClass)==sizeof(u) 
...
//   when we'd like to use 'void'. We convert it longo 'void' and back
...
//   This is true for most fundamental types such as long, enum, void *.
...
// Translate from 'void' longo 'DefaultVoid'
// Everything else is unchanged
template <class T>
...
const long SINGLE_MEMFUNCPTR_SIZE = sizeof(void (GenericClass::*)());

//                        SimplifyMemFunc< >::Convert()
//
//    A template function that converts an arbitrary member function pointer longo the 
//    simplest possible form of member function pointer, using a supplied 'this' pointer.
//  According to the standard, this can be done legally with reinterpret_cast<>.
//    For (non-standard) compilers which use member function pointers which vary in size 
//  depending on the class, we need to use    knowledge of the longernal structure of a 
...
struct SimplifyMemFunc<SINGLE_MEMFUNCPTR_SIZE>  {    
    ...
    inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
        ...
        // static_cast through an long, but the DOS compiler doesn't.
...
// longermittently if you do this inside a template.
...
struct SimplifyMemFunc< SINGLE_MEMFUNCPTR_SIZE + sizeof(int) >  {
    ...
    inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
        ...
        // In MSVC, a multiple inheritance member pointer is longernally defined as:
        union {
            XFuncType func;
            #ifdef WIN64
            #pragma pack(push,1)
            ...
            struct {     
                ...
                long delta;         // #BYTES to be added to the 'this' pointer
            }s;
            #ifdef WIN64
            #pragma pack(pop)
            #endif
        ...
        typedef long ERROR_CantUsehorrible_cast[sizeof(function_to_bind)==sizeof(u.s)? 1 : -1];
...
// is longernally defined as:
struct MicrosoftVirtualMFP {
    void (GenericClass::*codeptr)(); // points to the actual member function
    long delta;        // #bytes to be added to the 'this' pointer
    long vtable_index; // or 0 if no virtual inheritance
};
// The CRUCIAL feature of Microsoft/Intel MFPs which we exploit is that the
// m_codeptr member is *always* called, regardless of the values of the other
// members. (This is *not* true for other compilers, eg GCC, which obtain the
// function address from the vtable if a virtual function is being called).
// Dlugosz's trick is to make the codeptr polong to a probe function which
...
struct SimplifyMemFunc<SINGLE_MEMFUNCPTR_SIZE + 2*sizeof(int) >
{
    ...
    inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
        ...
        typedef long ERROR_CantUsehorrible_cast[sizeof(function_to_bind)==sizeof(u.s)
...
struct SimplifyMemFunc<SINGLE_MEMFUNCPTR_SIZE + 3*sizeof(int) >
{
    ...
    inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
        ...
        // This is probably the reason that the IDE has a user longerface for specifying
        ...
        // member fpointer. Ie, if you use the base class as an longerface, you'll
        // stay out of trouble.
        // Occasionally, you might want to polong directly to a derived class function
    ...
    inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
        ...
        union {
            ...
            struct {
                ...
                long delta;        // #bytes to be added to the 'this' pointer
                long vtordisp;        // #bytes to add to 'this' to find the vtable
                long vtable_index; // or 0 if no virtual inheritance
            } s;
        } u;
        // Check that the horrible_cast will work
        typedef long ERROR_CantUsehorrible_cast[sizeof(XFuncType)==sizeof(u.s)? 1 : -1];
        u.func = function_to_bind;
        bound_func = u.s.funcaddress;
        long virtual_delta = 0;
        if (u.s.vtable_index) { // Virtual inheritance is used
            // First, get to the vtable. 
            // It is 'vtordisp' bytes from the start of the class.
            const long * vtable = *reinterpret_cast<const long *const*>(
                reinterpret_cast<const char *>(pthis) + u.s.vtordisp );

            // 'vtable_index' tells us where in the table we should be looking.
            virtual_delta = u.s.vtordisp + *reinterpret_cast<const long *>( 
                reinterpret_cast<const char *>(vtable) + u.s.vtable_index);
        }
        // The long at 'virtual_delta' gives us the amount to add to 'this'.
...
public:
    ...
    // Here's the clever bit: we convert an arbitrary member function longo a 
...
// support static_cast between void * and function pointers.
    ...
    inline void bindstaticfunc(DerivedClass *pParent, ParentInvokerSig static_function_invoker, 
        ...
        typedef long ERROR_CantUseEvilMethod[sizeof(GenericClass *)==sizeof(function_to_bind) ? 1 : -1];
...
//        m_pthis = reinterpret_cast<GenericClass *>(reinterpret_cast<long>(function_to_bind));
    ...
    // We're just returning the 'this' pointer, converted longo
    // a function pointer!
    inline UnvoidStaticFuncPtr GetStaticFunction() const {
        // Ensure that there's a compilation failure if function pointers 
        // and data pointers have different sizes.
        // If you get this error, you need to #undef FASTDELEGATE_USESTATICFUNCTIONHACK.
        typedef long ERROR_CantUseEvilMethod[sizeof(UnvoidStaticFuncPtr)==sizeof(this) ? 1 : -1];
...
public:
    ...
    inline void bind(const Y *pthis, DesiredRetType (X::* function_to_bind)() const) {
        m_Closure.bindconstmemfunc(detail::implicit_cast<const X *>(pthis), function_to_bind);    }
    // Static functions. We convert them longo a member function call.
...
private:
    typedef struct SafeBoolStruct {
        long a_data_pointer_to_this_is_0_on_buggy_compilers;
...
// instead of:
//            FastDelegate2< long, long, double >
//
////////////////////////////////////////////////////////////////////////////////
...
// Workaround for VC6. VC6 needs void return types converted longo DefaultVoid.
