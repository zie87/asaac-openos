...
void SemaphoreManager::initialize()
{
	...
	for ( unsigned int Index = 0; Index < m_NumberOfSemaphores; Index ++ )
	{
...
int SemaphoreManager::findSemaphoreByName( const ASAAC_CharacterSequence& SemaphoreName )
{
	unsigned int Index;
...
int SemaphoreManager::findSemaphoreByPrivateId( const ASAAC_PrivateId SemaphoreId )
{
	unsigned int Index;
										  				...
										  				ASAAC_QueuingDiscipline Discipline )
{
	...
	int Index = findSemaphoreByName( Name );
...
ASAAC_ReturnStatus SemaphoreManager::deleteSemaphore( ASAAC_PrivateId SemaphoreId )
{
	...
	int Index = findSemaphoreByPrivateId( SemaphoreId );
