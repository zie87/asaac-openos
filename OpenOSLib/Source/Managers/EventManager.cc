...
void EventManager::initialize()
{
	...
	for ( unsigned int Index = 0; Index < m_NumberOfEvents; Index ++ )
	{
...
int EventManager::findEventByName( const ASAAC_CharacterSequence& EventName )
{
	unsigned int Index;
...
int EventManager::findEventByPrivateId( const ASAAC_PrivateId EventId )
{
	unsigned int Index;
...
ASAAC_ReturnStatus EventManager::deleteEvent( ASAAC_PrivateId EventId )
{
	...
	int Index = findEventByPrivateId( EventId );
