...
FileManager::FileManager()
{
	...
	for ( int Index = 0; Index < OS_MAX_NUMBER_OF_FILES; Index ++ )
	{
...
ASAAC_ReturnStatus FileManager::openFile( const ASAAC_CharacterSequence& Name, FileType Type, int Flags, int& FileHandle ) 
{
    try
    {
        CharSeq ErrorString;
		...
		try
		{
			int iFileFlags = oal_fcntl( FileHandle, F_GETFD );
		...
		// Enter new file data into File table
		FreeBlock->Name   = Name;
...
ASAAC_ReturnStatus FileManager::closeFile( const ASAAC_CharacterSequence& Name, int Flags )
{
	try
	{
...
ASAAC_ReturnStatus FileManager::closeFile( int Handle )
{
...
ASAAC_ReturnStatus FileManager::removeFile( int FileHandle )
{
...
int FileManager::getFileHandle( const ASAAC_CharacterSequence& Name, FileType Type, int Flags, long Size )
{
    int FileHandle = -1;
...
ASAAC_ResourceReturnStatus FileManager::createFile(const ASAAC_CharacterSequence name, const ASAAC_AccessRights access, const unsigned long file_size)
{
    try 
    {
        ...
        int Handle = creat(Name.c_str(), AccessRightsToMode( access ));
...
{
	throw OSException("APOS call not yet implemented.", LOCATION);
	return ASAAC_ERROR;
}
...
ASAAC_ReturnStatus FileManager::saveState( int& SavedStateHandler )
{
	try
	{
...
ASAAC_ReturnStatus FileManager::restoreStateByHandle( int SavedStateHandler )
{
	try
	{
...
FileManager::FileInfoBlock* FileManager::findFileByName( const ASAAC_CharacterSequence& Name, int Flags )
{
    if (Name.size == 0)
        return 0;
...
FileManager::FileInfoBlock* FileManager::findFileByHandle( int Handle )
{
	for ( int Index = 0; Index < OS_MAX_NUMBER_OF_FILES; Index ++ )
	{
...
ASAAC_ReturnStatus FileManager::restoreStateByName ( ASAAC_CharacterSequence File )
{
  	try
  	{
    	int SavedStateHandler = oal_open(CharSeq(File).c_str(), O_RDONLY,  S_IRUSR );
