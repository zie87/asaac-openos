...
void Process::initialize( bool IsServer, bool IsMaster, bool UseInternalCommandInterface, const ASAAC_ProcessDescription& Description, MemoryLocation Location )
{
	...
	for ( unsigned int Index = 0; Index < OS_MAX_NUMBER_OF_THREADS; Index++ )
	{
	...
	if ( IsMaster )
	{
		...
		for ( unsigned int Index = 0; Index < OS_MAX_NUMBER_OF_LOCALVCS; Index ++ )
		{
			m_LocalVCs[ Index ].GlobalVcId   = OS_UNUSED_ID;
			m_LocalVCs[ Index ].LocalVcId    = OS_UNUSED_ID;
		}

		for ( unsigned int Index = 0; Index < OS_MAX_NUMBER_OF_ENTRYPOINTS; Index ++ )
		{
...
ASAAC_TimedReturnStatus Process::launch()
{
	...
	try
	{
		...
		CharacterSequence ProcessStarterPath = FileNameGenerator::getAsaacPath(CharacterSequence(OS_PROCESS_STARTER).asaac_str());
		...
		SpawnArgs[0] = ProcessPathBuffer;
		SpawnArgs[1] = CpuIdBuffer;
		SpawnArgs[2] = ProcessIdBuffer;
		SpawnArgs[3] = 0;
			
		if ( oal_spawn( &m_PID, ProcessStarterPathBuffer, 0, 0, SpawnArgs, SpawnEnv ) != 0 )
...
ASAAC_ReturnStatus Process::destroy()
{
	...
	try
	{
		...
		if (TerminationStatus == ASAAC_TM_ERROR)
			OSException((ErrorString << "Termination command could not be send properly (PID=" << CharSeq(getId()) << ")").c_str(), LOCATION).logMessage();

		if (TerminationStatus == ASAAC_TM_TIMEOUT)
			OSException((ErrorString << "Termination command was not replied by application to be terminated (PID=" << CharSeq(getId())<<")").c_str(), LOCATION).logMessage();

		if (Data.Return == ASAAC_ERROR)
			OSException((ErrorString << "Application created an error while terminating itself. (PID=" << CharSeq(getId()) << ")").c_str(), LOCATION).logMessage();
		...
		if (P->getId() != this->getId())
		{
			...
			{	
				OSException("Signal to assume processes readyness could not be sent.", LOCATION).logMessage();
			}
			...
			{	
				...
				else OSException("Kill Signal cannot be send, because pid is unknown (PID=0).", LOCATION).logMessage();
			}
