 ...
 *  done via direct access to shared memory without required interaction
...
public:
	...
	/*!< this function adds an entry point to the table of entry points stored
	 ...
	 * 
	 *   \param[in] Name ASAAC_CharacterSequence identifying the entry point descriptively
	 *   \param[in] ASAAC_Address Jump address to the function constituting the thread.
	 * 
	 *   \returns ASAAC_SUCCESS if the thread entry point could successfully be added to
	...
	/*!< retrieve the entry address for an entry point stored for this process.
	 * 
	 *   \param[in] Name Name of the entry point
	 *   \returns   Entry point address. 0, if entry point was not found.
	 */
...
private:
	 ...
	 *   the intended mechanism via a MasterProcess/ProcessManager makes use of functionality
