...
public:
	 ...
	 *   and sets all internal configuration sets to respectively
	 *   required values.
	 * 
	 ...
	 *   the thread's entry point to be declared at the time of
	 *   thread creation. However, trying to run the thread will fail
	 *   of no respective entry point is configured.
	 * 
	 ...
	 *   point as assigned to this Thread instance and it sets the thread state
	 *   to RUNNING.
	 * 
	 ...
	 *   point where it is safe to cancel execution without blocking OS resources shared
	 ...
	 *   specification. The current implementation translates suspendSelf() into
...
private:
	...
	unsigned int		m_ProtectedScopeStackSize;
