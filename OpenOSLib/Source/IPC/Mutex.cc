...
Mutex::Mutex( Allocator* ThisAllocator, bool IsMaster, int Protocol ) : m_IsInitialized(false)
{
...
void Mutex::initialize( Allocator* ThisAllocator, bool IsMaster, int Protocol )
{
...
ASAAC_TimedReturnStatus Mutex::lock( const ASAAC_Time& Timeout )
{
	...
	int iError = oal_thread_mutex_timedlock( &( Global->Mutex ), &TimeSpecTimeout );
...
ASAAC_ReturnStatus Mutex::release()
{
	...
	int iError = oal_thread_mutex_unlock( &(Global->Mutex) );
