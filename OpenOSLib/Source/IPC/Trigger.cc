...
unsigned int Trigger::getTriggerState()
{
	return m_Global->TriggerValue;
}


ASAAC_TimedReturnStatus Trigger::waitForTrigger( unsigned int& TriggerState, const ASAAC_TimeInterval& Timeout )
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException( LOCATION );
...
{
	unsigned int TriggerState = getTriggerState();
...
unsigned int Trigger::getWaitCount()
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException( LOCATION );
