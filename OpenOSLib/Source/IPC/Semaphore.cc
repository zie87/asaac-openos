...
Semaphore::Semaphore(Allocator* ThisAllocator, const bool IsMaster, const int InitialCount, const int MaximumCount ) : m_IsInitialized(false)
{
...
void Semaphore::initialize(Allocator* ThisAllocator, const bool IsMaster, const int InitialCount, const int MaximumCount )
{
...
{	
	...
	// oal_sem_wait can be interrupted by signals and in this case returns with
	// a nonzero result.
...
ASAAC_ReturnStatus Semaphore::post()
{
	...
	if ( int(getCount()) == m_Global->MaximumCount )
	{
...
unsigned int Semaphore::getCount() const
{
	...
	if ( oal_sem_getvalue( &(m_Global->Semaphore), &iReturn ) != 0 ) throw OSException( LOCATION );
	
	return iReturn;
}


unsigned int Semaphore::getWaitingThreads() const
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException( LOCATION );
