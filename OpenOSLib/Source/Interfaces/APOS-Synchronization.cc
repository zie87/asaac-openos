                                                  ...
                                                  ASAAC_PrivateId* event_id)
{
	try
	{
		EventManager::getInstance()->createEvent( *name, *event_id );
    }
    catch ( ASAAC_Exception &e )
    {
        e.addPath("APOS::createEvent", LOCATION);
        e.raiseError();
...
ASAAC_ReturnStatus ASAAC_APOS_deleteEvent( const ASAAC_PrivateId event_id )
{
	try
	{
		EventManager::getInstance()->deleteEvent( event_id );
    }
    catch ( ASAAC_Exception &e )
    {
        e.addPath("APOS::deleteEvent", LOCATION);
        e.raiseError();
...
ASAAC_ReturnStatus ASAAC_APOS_setEvent( const ASAAC_PrivateId event_id )
{
	try
	{
		EventManager::getInstance()->setEvent( event_id );
    }
    catch ( ASAAC_Exception &e )
    {
        e.addPath("APOS::setEvent", LOCATION);
        e.raiseError();
...
ASAAC_ReturnStatus ASAAC_APOS_resetEvent( const ASAAC_PrivateId event_id )
{
	try
	{
		EventManager::getInstance()->resetEvent( event_id );
    }
    catch ( ASAAC_Exception &e )
    {
        e.addPath("APOS::resetEvent", LOCATION);
        e.raiseError();
                                ...
                                const ASAAC_TimeInterval* timeout )
{
	try
	{
		EventManager::getInstance()->waitForEvent( event_id, TimeStamp(*timeout).asaac_Time() );
    }
    catch ( ASAAC_Exception &e )
    {
        e.addPath("APOS::waitForEvent", LOCATION);
        e.raiseError();
                                             ...
                                             unsigned long* waiting_callers)                          
{
	try
	{
		EventManager::getInstance()->getEventStatus( event_id, *event_status, *waiting_callers );
    }
    catch ( ASAAC_Exception &e )
    {
        e.addPath("APOS::getEventStatus", LOCATION);
        e.raiseError();
                                         ...
                                         ASAAC_PrivateId* event_id)
{
	try
	{
		EventManager::getInstance()->getEventId( *name, *event_id );
    }
    catch ( ASAAC_Exception &e )
    {
        e.addPath("APOS::getEventId", LOCATION);
        e.raiseError();
