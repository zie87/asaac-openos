...
public:
	SharedMemory( const ASAAC_CharacterSequence& Name, bool IsMaster, unsigned int Size );
	//!< instantiate allocator and initialize it instantly (see initialize()).

	SharedMemory();
	
	void initialize( const ASAAC_CharacterSequence& Name, bool IsMaster, unsigned int Size, bool EraseMemory = true, bool EvaluateSession = true );
	...
	/*!< unmap the shared memory object from memory and reset internal data structures of the
