...
LocalMemory::LocalMemory(int Size) : m_IsInitialized(false), m_BaseAddress(0),m_MemorySize(0), m_UsedMemory(0)
{
	initialize( Size );
}
...
void LocalMemory::initialize( int Size )
{
...
ASAAC_Address LocalMemory::allocate( int Size )
{
	if (m_IsInitialized == false) 
		throw UninitializedObjectException(LOCATION);

	// If new total memory exceeds allocatable size, throw Exception
	if ( Size + m_UsedMemory > m_MemorySize ) throw ResourceException();
...
unsigned int LocalMemory::getSize() const
{
	if (m_IsInitialized == false) 
		throw UninitializedObjectException(LOCATION);
...
unsigned int LocalMemory::getFreeMemory() const
{
	if (m_IsInitialized == false) 
		throw UninitializedObjectException(LOCATION);
...
unsigned int LocalMemory::getUsedMemory() const
{
	if (m_IsInitialized == false) 
		throw UninitializedObjectException(LOCATION);
