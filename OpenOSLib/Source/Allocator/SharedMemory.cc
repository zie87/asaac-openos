...
SharedMemory::SharedMemory( const ASAAC_CharacterSequence& Name, bool IsMaster, unsigned int Size ) 
		: 
...
void SharedMemory::initialize( const ASAAC_CharacterSequence& Name, bool IsMaster, unsigned int Size, bool EraseMemory, bool EvaluateSession )
{
	if ( m_IsInitialized ) 
...
void SharedMemory::deinitialize()
{
	...
	{
		CharSeq ErrorString;
		FatalException( (ErrorString << "Error deinitializing SharedMemory: " << m_Name << " [" << m_BaseMemorySize << "]").c_str(), LOCATION).logMessage();
	}
...
ASAAC_Address SharedMemory::allocate( int Size )
{
	if (m_IsInitialized == false) 
	...
	if ( (unsigned int)(ActualSize + m_UsedMemory) > getMemoryHeader()->Size ) 
		throw OSException("Allocation Error", LOCATION);
	
	// return address of next available memory slot
	// (the first unsigned int is used for allocation count)
...
unsigned int SharedMemory::getFreeMemory() const
{
	if (m_IsInitialized == false) 
		throw UninitializedObjectException(LOCATION);
...
unsigned int SharedMemory::getUsedMemory() const
{
	if (m_IsInitialized == false) 
		throw UninitializedObjectException(LOCATION);
