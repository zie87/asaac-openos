...
ASAAC_ReturnStatus LocalVc::assign( const ASAAC_VcMappingDescription& Description )
{
	...
	try
	{
		...
		*m_Description = Description;
		...
		unsigned int ThisQueueSize = m_Description->number_of_message_buffers;
...
ASAAC_ReturnStatus LocalVc::unassign()
{
	...
	try
	{
		...
		try 
		{
			...
			for (;;)
			{
				unsigned int Buffer = m_Queue.pop( TimeStamp::Instant().asaac_Time() );
								  ...
								  const ASAAC_Time& Timeout )
{
	...
	try
	{
		...
		unsigned int BufferId = m_ParentGlobalVc->getBufferNumber( BufferReference );
										  ...
										  const ASAAC_Time& Timeout )
{
	...
	try 
    {
		...
		unsigned int BufferId = m_Queue.pop( Timeout );
...
ASAAC_TimedReturnStatus LocalVc::queueBuffer( unsigned int BufferNumber, const ASAAC_Time& Timeout )
{
    if ( m_IsInitialized == false ) 
        throw UninitializedObjectException(LOCATION);
...
size_t LocalVc::predictSize( unsigned int MaximumBuffers )
{
	return ( Shared<ASAAC_VcMappingDescription>::predictSize() +
			 SharedCyclicQueue<unsigned int>::predictSize( MaximumBuffers ) ) ;
}
