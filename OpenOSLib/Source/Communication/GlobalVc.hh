...
const int MAX_USAGE_COUNT = 32767;
const int UNDEF_USAGE_COUNT = -1;
...
typedef struct {
	unsigned int	NumberOfConnectedVCs;
	unsigned int	NumberOfConnectedTCs;
...
public:
	 ...
	 *                       shall be the master of the internal data structures of
	 ...
	 *   sending or receiving via it. The internal data structures are reset into
	...
	int	     getLocalVcIndex( ASAAC_PublicId ProcessId, ASAAC_PublicId LocalVcId );
	...
	LocalVc* getLocalVcByIndex( int Index );
	...
	void claimBuffer( unsigned int Buffer );
	...
	void releaseBuffer(unsigned int Buffer);
	...
	BufferInfo* getBufferInfo(unsigned int Buffer) const;
	...
	ASAAC_Address getBufferArea(unsigned int Buffer) const;
	...
	int getBufferNumber( ASAAC_Address BufferLocation ) const;
	...
	ASAAC_TimedReturnStatus sendBuffer( unsigned int SourceBuffer, const ASAAC_Time& Timeout ); 
	 ...
	 *                           has been written into the memory area indicated by getBufferArea(), and the
...
protected:	
	...
	SharedCyclicQueue<unsigned int>	 m_FreeBuffersQueue;
