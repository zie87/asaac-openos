...
public:
	 ...
	 *   \param[in] ThisAllocator  Reference to the Allocator that shall be used for the internal data structures
	 *                             of the LocalVc
	 ...
	 *                             callers via SMOS interfaces should act as master.
	 ...
	 * 
	 *  Note: The initialization only sets up the internal structures and allocates memory for the operation
    ...
    //! Copy a message into a buffer and send on a VC.
     ...
     * data from operating system memory into the buffer in the caller
     ...
     * 
     * \param[in] BufferReference Location of a buffer into which the received message is
     *                            to be copied.
     ...
     * calling thread is transferred into a waiting state. The thread
     * will be transferred into a ready state when the buffer has
     ...
     * message data into this buffer for sending.  The OS shall
     * lock a buffer of sufficient size.
     * 
     * If a buffer is not immediately available, the calling thread is
     * transferred into WAITING state. It will be passed on to
     ...
     * \param[in]  Timeout         Maximum interval to wait for a buffer to become
     *                                 available.
     * 
     * \return	ASAAC_TM_SUCCESS on successful completion.
     ...
     * the calling thread is transferred into WAITING state. The calling
     ...
     * \param[in]  Timeout          Maximum interval to wait for a message to become
     *                                 available.
     * 
     * \return	ASAAC_TM_SUCCESS on successful completion.
     * 			ASAAC_TM_TIMEOUT if no buffer was available within the time specified by the Timeout.
	...
	/*!<  transfer the calling thread into WAITING state until either data
	 *    is available to be read for the LocalVc, or the Timeout has elapsed.
	 *    After that, the thread is returned into RUNNING state.
	 * 
	...
	/*!< transfers the calling thread into WAITING state until either a
	 *   free slot is available for writing data into this LocalVc, or the Timeout
	 *   has elapsed. After that, the thread is returned into RUNNING state.
	 * 
	 * \param[in] Timeout Maximum Timeinterval to wait before returning with a ASAAC_TM_TIMEOUT
	 * 
	...
	//! push one buffer index number into the receiver queue of this local vc
	ASAAC_TimedReturnStatus queueBuffer( unsigned int BufferNumber, const ASAAC_Time& Timeout = TimeInfinity );
	 ...
	 *  is pushed into the LocalVc's queue.
	 * 
	...
	static size_t predictSize( unsigned int MaximumBuffers );
...
public:
    ...
    SharedCyclicQueue<unsigned int>  	m_Queue;
