...
void GlobalVc::initialize(ASAAC_PublicId GlobalVc, bool IsMaster, const ASAAC_VcDescription& Data, SessionId p_SessionId )
{
	...
	{	
		...
		unsigned int TotalNumberOfBuffers = m_Description->max_number_of_buffers;
	
	
		m_BufferInfo.initialize( &m_Allocator, TotalNumberOfBuffers );
		
		// Allocate shared memory for buffer data
		unsigned int TotalBufferSize = TotalNumberOfBuffers * ( m_Description->max_msg_length + sizeof(unsigned int) );
		...
		if ( m_IsMaster )
		{
			...
			for ( unsigned int i = 0; i < TotalNumberOfBuffers; i++ )
			{
				// Write Buffer number into the memory cell just in front of the actual buffer area
				((unsigned int*)getBufferArea( i ))[ -1 ] = i;
			}
		}
		...
		for ( unsigned int i = 0; i < m_Description->max_number_of_threads_attached; i++ )
		{	
...
void GlobalVc::deinitialize()
{
	...
	try
	{
		...
		for ( unsigned int i = 0; i < m_DescriptionBackup.max_number_of_threads_attached; i++ )
		{
...
ASAAC_ReturnStatus GlobalVc::createLocalVc( const ASAAC_VcMappingDescription& Description )
{
	...
	try
	{
		...
		int NewLocalVcIndex = getLocalVcIndex( OS_UNUSED_ID, OS_UNUSED_ID );
		...
		if ( Description.is_reading == false )
		{
			...
			{
				unsigned int FreeBuffer = getFreeBuffer();
...
ASAAC_ReturnStatus GlobalVc::removeLocalVc( ASAAC_PublicId ProcessId, ASAAC_PublicId LocalVcId )
{
    ...
    try
    {
		...
		int ThisLocalVcIndex = getLocalVcIndex( ProcessId, LocalVcId );
...
BufferInfo* GlobalVc::getBufferInfo( unsigned int Buffer ) const
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException(LOCATION);
...
ASAAC_Address GlobalVc::getBufferArea( unsigned int Buffer ) const
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException(LOCATION);
	...
	unsigned long BufferUnit = m_Description->max_msg_length + sizeof(unsigned int ); 
	
	return static_cast<ASAAC_Address>( &(m_BufferData[ BufferUnit * Buffer + sizeof(unsigned int)]) );
}


int GlobalVc::getBufferNumber( ASAAC_Address BufferLocation ) const
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException(LOCATION);
	...
	if ( BufferLocation != 0 )
	{
		unsigned int iGuessedNumber = ((unsigned int*)BufferLocation)[-1];
...
ASAAC_TimedReturnStatus GlobalVc::sendBuffer( unsigned int SourceBuffer, const ASAAC_Time& Timeout )
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException(LOCATION);
    ...
    try
    {
		...
		for ( int iVC = 0; iVC < (int)m_Description->max_number_of_threads_attached; iVC++ )
		{
		...
		if ( SenderLocalVc != 0 )
		{
			// push more free buffer into sender queue
			unsigned int NextFreeBuffer = getFreeBuffer();
...
unsigned int GlobalVc::getFreeBuffer()
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException(LOCATION);
...
void GlobalVc::claimBuffer( unsigned int BufferNumber )
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException(LOCATION);
...
void GlobalVc::releaseBuffer( unsigned int BufferNumber )
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException(LOCATION);
...
LocalVc* GlobalVc::getLocalVc( ASAAC_PublicId ProcessId, ASAAC_PublicId LocalVcId )
{
	...
	int Index = getLocalVcIndex( ProcessId, LocalVcId );
...
int	GlobalVc::getLocalVcIndex( ASAAC_PublicId ProcessId, ASAAC_PublicId LocalVcId )
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException(LOCATION);

	for ( unsigned int Index = 0; Index < m_Description->max_number_of_threads_attached; Index ++ )
	{
...
int	GlobalVc::getSendingLocalVcIndex()
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException(LOCATION);
...
LocalVc* GlobalVc::getLocalVcByIndex( int Index )
{
	if ( m_IsInitialized == false ) 
		throw UninitializedObjectException(LOCATION);

	if ( (  Index < 0 ) || 
         ( (unsigned int)Index > m_Description->max_number_of_threads_attached ) ) 
		return 0;
...
size_t GlobalVc::predictSize( const ASAAC_VcDescription& Description )
{
	...
	unsigned int TotalNumberOfBuffers = Description.max_number_of_buffers *
	...
	unsigned int TotalBufferSize = TotalNumberOfBuffers * ( Description.max_msg_length + sizeof(unsigned int) );

	// m_BufferData
	CumulativeSize += Shared<char>::predictSize( TotalBufferSize );

	// m_FreeBuffersQueue
	CumulativeSize += SharedCyclicQueue<unsigned int>::predictSize( TotalNumberOfBuffers ); 
